...
bool DNSResolver::exchangeData(const fd_set &rdfds, const fd_set &wrfds) {
...
void DNSResolver::receiveAndParse() {
...
void DNSResolver::fillFdSet(fd_set &rdfds, fd_set &wrfds) {
	if (!requestSended) {
		FD_SET(sockfd, &wrfds);
